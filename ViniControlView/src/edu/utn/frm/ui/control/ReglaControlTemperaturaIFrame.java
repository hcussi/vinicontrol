/*
 * ReglaControlTemperaturaIFrame.java
 *
...
import edu.utn.frm.action.reglacontrol.ReglaControlTemperaturaAbm;
 ...
 * @author Proyecto
 */
public class ReglaControlTemperaturaIFrame extends javax.swing.JInternalFrame {

    private GenericTableModel<ReglaControlTemperatura> model = new GenericTableModel<ReglaControlTemperatura>(
            new String[]{"Código", "Descripción", "Temperatura Mínima", "Temperatura Máxima", "Temperatura Tolerancia"},
            new String[]{"codigo", "descripcion", "temperaturaMinima", "temperaturaMaxima", "temperaturaTolerancia"});
    private ReglaControlTemperaturaAbm gestorAbm = new ReglaControlTemperaturaAbm();

    /** Creates new form FincaIFrame */
    public ReglaControlTemperaturaIFrame() {
        initComponents();
    ...
    private void initComponents() {
        ...
        datosPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Datos Regla de Control de Temperatura"));
        ...
        temperaturaMinimaText.setName("txtTCapacidad"); // NOI18N

        binding = org.jdesktop.beansbinding.Bindings.createAutoBinding(org.jdesktop.beansbinding.AutoBinding.UpdateStrategy.READ_WRITE, this, org.jdesktop.beansbinding.ELProperty.create("${entity.detalle}"), temperaturaMinimaText, org.jdesktop.beansbinding.BeanProperty.create("text"));
        bindingGroup.addBinding(binding);
        ...
        temperaturaMaximaText.setName("txtTModelo"); // NOI18N

        temperaturaToleranciaText.setBorder(javax.swing.BorderFactory.createEtchedBorder());
        temperaturaToleranciaText.setName("txtTDescripcion"); // NOI18N
        ...
        setIconifiable(true);
        setTitle("Regla Control de Temperatura");

        sPanel.setBorder(javax.swing.BorderFactory.createTitledBorder("Reglas de Control de Temperatura"));
    ...
    private void editar() {
        ReglaControlTemperatura reglaControlTemperatura = null;

        if (buscarTabla.getSelectedRow() != -1) {
            reglaControlTemperatura = model.getRow(buscarTabla.getSelectedRow());
            if (reglaControlTemperatura != null) {
                gestorAbm.setEntity(reglaControlTemperatura);
                bindinDataEntity(servidor);
                habilitarPanelDatos(true);
                showDatosDialog(true);
            }
    ...
    private void guardar() {
        ...
        try {
            temperaturaMinima = new Double(temperaturaMinimaText.getText());
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(datosDialog, "La temperatura mínima tiene un formato incorrecto.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        try {
            temperaturaMaxima = new Double(temperaturaMaximaText.getText());
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(datosDialog, "La temperatura máxima tiene un formato incorrecto.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        try {
            new Double(temperaturaToleranciaText.getText());
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(datosDialog, "La temperatura tolerancia tiene un formato incorrecto.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if (temperaturaMaxima < temperaturaMinima) {
            JOptionPane.showMessageDialog(datosDialog, "La temperatura máxima debe ser mayor a la mínima", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
    ...
    public ReglaControlTemperaturaAbm getGestorAbm() {
        return gestorAbm;
    }

    public void setGestorAbm(ReglaControlTemperaturaAbm gestorAbm) {
        this.gestorAbm = gestorAbm;
    }

    public ReglaControlTemperatura getEntity() {
        return gestorAbm.getEntity();
    }

    public void setEntity(ReglaControlTemperatura entity) {
        this.gestorAbm.setEntity(entity);
    }

    private void habilitarPanelDatos(boolean habilitar) {
        codigoText.setEnabled(habilitar);
        temperaturaMinimaText.setEnabled(habilitar);
        descripcionText.setEnabled(habilitar);
        temperaturaMaximaText.setEnabled(habilitar);
        temperaturaToleranciaText.setEnabled(habilitar);
    ...
    public void bindinDataEntity(int fuente) {
        if (fuente == servidor) {
            ...
            temperaturaMinimaText.setText(getEntity().getTemperaturaMinima().toString());
            temperaturaMaximaText.setText(getEntity().getTemperaturaMaxima().toString());
            temperaturaToleranciaText.setText(getEntity().getTemperaturaTolerancia().toString());
            ...
            getEntity().setTemperaturaMaxima(new Double(temperaturaMaximaText.getText()));
            getEntity().setTemperaturaMinima(new Double(temperaturaMinimaText.getText()));
            getEntity().setTemperaturaTolerancia(new Double(temperaturaToleranciaText.getText()));
        }
